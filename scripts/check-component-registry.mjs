...
for (const duplicate of new Set(duplicateValues(components.map(({ name }) => name)))) {
  errors.push(`Duplicate component name: ${duplicate}`)
}
for (const duplicate of new Set(duplicateValues(components.map(({ slug }) => slug)))) {
  errors.push(`Duplicate component slug: ${duplicate}`)
}
for (const duplicate of new Set(duplicateValues(components.map(({ source }) => source)))) {
...
for (const component of components) {
  ...
  try {
    ...
    if (!sourceStat.isFile()) errors.push(`Source is not a file: ${component.source}`)
  } catch {
...
const registeredUiFiles = components
  .map(({ source }) => source)
  .filter((source) => source.startsWith("src/components/ui/") && source.endsWith(".tsx"))
  .sort()

for (const source of uiFiles) {
  if (!registeredUiFiles.includes(source)) errors.push(`Unregistered public UI source: ${source}`)
}
for (const source of registeredUiFiles) {
  if (!uiFiles.includes(source)) errors.push(`Registry references unknown UI source: ${source}`)
}
...
for (const slug of moduleExportKeys) {
  if (!registeredSlugs.includes(slug)) errors.push(`Unregistered module export: ./${slug}`)
}
for (const slug of registeredSlugs) {
  if (!moduleExportKeys.includes(slug)) errors.push(`Registry slug has no module export: ./${slug}`)
}

const sorted = [...components].sort((a, b) => a.slug.localeCompare(b.slug))
const rows = sorted.map(
  ({ name, slug, category, status, client, interactive }) =>
    `| \`@mivama/ui/${slug}\` | ${name} | ${category} | ${status} | ${client ? "Yes" : "No"} | ${interactive ? "Yes" : "No"} |`
)
const generated = `# Package exports\n\nThis file is generated from \`config/components.mjs\`. Do not edit it manually.\n\n| Import | Primary component | Category | Status | Client boundary | Interactive |\n|---|---|---|---|---|---|\n${rows.join("\n")}\n`
...
if (writeMode) {
  ...
  try {
    ...
    errors.push("Missing generated export documentation: docs/generated/exports.md")
  }
  if (existing && existing !== generated) {
    errors.push("Generated export documentation is stale. Run npm run registry:write.")
  }
}
